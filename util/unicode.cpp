...
ostream& codepage(ostream& os, uint32_t code)
{
    ...
    if (code < 0x10000) {
        os << "\\u";
        os << setfill('0') << setw(2) << hex << ((code & 0xff00) >> 8);
        os << setfill('0') << setw(2) << hex << (code & 0xff);
    } else {
        os << "\\U";
        os << setfill('0') << setw(2) << hex << ((code & 0xff0000) >> 16);
        os << setfill('0') << setw(2) << hex << ((code & 0xff00) >> 8);
        os << setfill('0') << setw(2) << hex << (code & 0xff);
    }
...
ostream& charAsHex(ostream& os, const string& chars)
{
    ...
    for (size_t i = 1; i < chars.size(); i++) {
        os << " " << setfill('0') << setw(2) << hex << static_cast<int>(static_cast<uint8_t>(chars[i]));
    }
    return os;
}
...
ostream& validChar2(ostream& os, uint8_t c)
{
    if (static_cast<char>(c) == '\\')
        ...
        util::IosFlagSaver iosFlagSaver(os);
        os << "\\u" << setfill('0') << setw(4) << hex << static_cast<int>(c);
    } else
        os << static_cast<char>(c);
