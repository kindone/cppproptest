...
decltype(auto) callTransform(Tuple&& tup) {
    return FunctionTemplate<typename std::tuple_element<N, Tuple>::type>::transform(std::forward<typename std::tuple_element<N,Tuple>::type>(std::get<N>(tup)));
}
...
decltype(auto) callTransformWithArg(Tuple&& tup, Arg&& arg) {
    return FunctionTemplate<typename std::tuple_element<N, Tuple>::type>::transform(std::forward<typename std::tuple_element<N, Tuple>::type>(std::get<N>(tup)), std::forward<Arg>(arg));
}
...
decltype( auto ) transformHeteroTupleWithArgHelper(Tuple&& tup, Arg&& arg, std::index_sequence<index...> index_sequence) {
    return std::make_tuple(callTransformWithArg<FunctionTemplate, Tuple, Arg, index>(std::forward<Tuple>(tup), std::forward<Arg>(arg))...);
}
